...
class TopicComponentFactory:
    ...
    def get_component(topic_component_name, topic_component_model=None, kwargs={}):
        if topic_component_name not in TopicComponentFactory.components_map:
            raise ValueError(
                "{} not a recognized topic component".format(topic_component_name)
            )

        return TopicComponentFactory.components_map[topic_component_name](
            model=topic_component_model, **kwargs
        )
