...
class DocumentTopicSummarizer:
    ...
    def __init__(self, openai_api_key):
        ...
        dimension_reduction_model = self.topic_components.get_component("dimensionality_reduction_model").model
        ...
        self.embed_model = self.topic_components.get_component("embedding_model",topic_component_model=self._set_up_embedding_model(openai_api_key)).model
        self.representation_model = {
            "KeyBERT": KeyBERTInspired(),
            "LLM": self.topic_components.get_component("topic_representation",kwargs={"api_key":openai_api_key}).model
        }
        ...
        self.topic_extractor = BERTopic(
            ...
            vectorizer_model=self.topic_components.get_component("topic_vectorizer").model,
    ...
    def embed_text(self,splits):
        ...
        reduced_embeddings = self.viz_dimension_reduction_model.fit_transform(all_embeddings)
    ...
    def generate_topics_probs(self, splits, embeddings):
        ...
        llm_labels = [re.sub(r'\W+', ' ', label[0][0].split("\n")[0].replace('"', '')) for label in self.topic_extractor.get_topics(full=True)["LLM"].values()]
        llm_labels = [label if label else "Unlabelled" for label in llm_labels]
        all_labels = [llm_labels[topic+self.topic_extractor._outliers] if topic != -1 else "Unlabelled" for topic in topics]
